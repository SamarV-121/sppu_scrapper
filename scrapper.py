...
def firestore_add(college):
    payload = {"college": college}
    ref = db.collection("college_info").document("college").set(payload)
    print(ref)


def firestore_get():
    return (
        db.collection("college_info").document("college").get().to_dict().get("college")
    )
